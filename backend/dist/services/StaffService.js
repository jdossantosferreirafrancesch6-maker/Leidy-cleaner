...
class StaffService {
     ...
     */
    static async getStaffBookings(staffId, status) {
        let sql = `
      SELECT 
     ...
     */
    static async getStaffDashboard(staffId) {
        const today = await (0, database_1.query)(`SELECT * FROM bookings 
       WHERE staff_id = $1 AND scheduled_date::date = CURRENT_DATE AND status != 'cancelled'
       ORDER BY scheduled_date ASC`, [staffId]);
        const upcoming = await (0, database_1.query)(`SELECT * FROM bookings 
     ...
     */
    static async addSpecialDate(staffId, startDate, endDate, type, reason) {
        const result = await (0, database_1.query)(`INSERT INTO special_dates 
       (staff_id, start_date, end_date, type, reason, created_at)
       VALUES ($1, $2, $3, $4, $5, NOW())
       RETURNING *`, [staffId, startDate, endDate, type, reason]);
        return result[0];
    }
    /**
     * Listar folgas especiais
     */
    static async getSpecialDates(staffId) {
        return await (0, database_1.query)(`SELECT * FROM special_dates 
       WHERE staff_id = $1 AND end_date >= CURRENT_DATE
       ORDER BY start_date ASC`, [staffId]);
    }
    /**
     * Listar staff com melhores ratings
     */
    static async getTopStaff(limit = 10) {
        return await (0, database_1.query)(`SELECT 
