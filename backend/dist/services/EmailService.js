...
transporter.verify((error, success) => {
    if (error) {
        logger_advanced_1.logger.error('❌ Erro na conexão SMTP:', error.message);
    }
    else {
        logger_advanced_1.logger.info('✅ Conexão SMTP OK');
    }
});
exports.EmailService = {
    ...
    async send(options) {
        try {
            ...
            logger_advanced_1.logger.info(`✅ Email enviado para ${options.to}`);
            return result;
        }
        catch (error) {
            logger_advanced_1.logger.error(`❌ Erro ao enviar email para ${options.to}:`, error);
            throw error;
        }
    },
    /**
     ...
     */
    async sendBookingConfirmation({ customerName, customerEmail, bookingId, serviceName, scheduledDate, address, totalPrice, notes, }) {
        const html = `
      <div style="font-family: Arial, sans-serif; padding: 20px;">
     ...
     */
    async sendPaymentReceipt({ email, transactionId, amount, method, date, bookingId, serviceName, }) {
        const html = `
      <div style="font-family: Arial, sans-serif; padding: 20px;">
     ...
     */
    async sendBookingCancelled(email, bookingId, serviceName, reason) {
        const html = `
      <div style="font-family: Arial, sans-serif; padding: 20px;">
     ...
     */
    async notifyStaffNewBooking(staffEmail, staffName, bookingDetails) {
        const html = `
      <div style="font-family: Arial, sans-serif; padding: 20px;">
     ...
     */
    async sendReviewReminder(email, customerName, bookingDetails) {
        const html = `
      <div style="font-family: Arial, sans-serif; padding: 20px;">
        <h2>Nos Avalie! ⭐</h2>
        <p>Olá ${customerName},</p>
